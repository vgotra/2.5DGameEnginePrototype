...
- On resize/fullscreen the sample also calls `VulkanRenderer.Resize`, which recreates the swapchain, image views, and framebuffers; the command pool, command buffers, semaphores, and frame-in-flight fences are recreated together on resize.
