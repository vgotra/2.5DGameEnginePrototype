...
- **MCP** servers live in `opencode.json` (opencode reads only that), with this `mcp.json` as the neutral catalog. Keep them in sync. Current servers: `vulkan` (Vulkan registry lookup, built by `tools/Setup-McpServers.ps1` into gitignored `tools/mcp/`), `renderdoc` (`.rdc` capture analysis via `uvx`, see `docs/RenderDocSetup.md`), and `mcp-repo-graph` (codebase structural graph via `uvx`, caches to `.ai/repo-graph/`). All three `uvx`/Python servers pin `mcp>=1.0,<2` — the `mcp` 2.x line dropped `mcp.server.fastmcp`, so newer `mcp` breaks them.
