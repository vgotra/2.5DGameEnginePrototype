...
- Frame-pacing target (planned, not implemented — roadmap item 1 / `docs/FramePacingPlan.md`): prefer `VK_PRESENT_MODE_MAILBOX_KHR` (fallback FIFO) with triple buffering; per-swapchain-image fences replacing the single in-flight fence and the per-frame `vkQueueWaitIdle`; high-resolution dt + frame cap; persistent dirty-gated vertex buffers; clean shutdown on ESC/window close.
