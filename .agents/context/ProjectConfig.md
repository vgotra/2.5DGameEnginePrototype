...
- Flags: `--2d` (flat top-down view), `--fullscreen` (borderless start), `--cap <fps>` (frame-rate cap), `--metrics` (rolling table every 120 frames), `--parallel` (force parallel tile extraction + draw recording), `--simulation` (128×128 map + 100k critter ECS stress test).
