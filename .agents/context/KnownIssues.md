...
- Vulkan frame delivery is less steady than GDI: the swapchain uses FIFO vsync (`presentMode = modes[0]`) with double buffering and a single in-flight fence, and every frame runs a staging upload + `vkQueueWaitIdle` before present — timing jitter becomes visible judder on camera pans where GDI's immediate `BitBlt` looks smooth. Planned fix tracked as roadmap item 1 / `docs/FramePacingPlan.md`.
