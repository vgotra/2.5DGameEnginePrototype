...
Completed: SDK/build policy, core entity/clock types, isometric math, ECS storage, worker-scheduler foundation, backend-neutral contracts (rendering/audio/physics/platform), deterministic tile map, movement + collision + jump + camera, Win32 window/input, Vulkan setup + full batched draw path via `IRenderer`, smoke tests, white/black tile styling (diamonds in iso, boxes in `--2d`), fullscreen switching (`F11`/`--fullscreen`) with Vulkan swapchain rebuild, `--2d` flat mode, the centered 800x600 window, map-bounds camera centering in fullscreen, and cross-platform platform seams (`IGameWindow`/`IInputState`/`PlatformKind`/`NativeWindowSurface`, `Engine.Platform.Desktop.GamePlatform` host, per-OS Vulkan loader/surface selection), and removal of the GDI reference renderer (Vulkan is the only renderer).
