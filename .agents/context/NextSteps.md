...
6. Optional: verify roadmap item #14 (`[LibraryImport]` migration) per `docs/LibraryImportVerificationPlan.md` — AOT publish (Tier B, the real non-functional proof), dotnet-counters/trace regression baseline (Tier A), BenchmarkDotNet microbench (Tier C, belongs to #5), or in-engine telemetry (Tier D, belongs to #5).
...
Pending (see `docs/Roadmap.md`): texture sampling, asset loading, ECS queries/system scheduling, profiling, job dependencies, scene/save format, audio backend, physics adapter, animation/tile atlas, debug tools, editor, Linux (SDL2) platform backend. The Win32 `[LibraryImport]` migration (roadmap item 14) is complete.
