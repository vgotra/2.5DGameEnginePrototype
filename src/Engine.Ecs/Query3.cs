...
public sealed class Query<T1, T2, T3> where T1 : unmanaged where T2 : unmanaged where T3 : unmanaged
{
    ...
    public void ForEach<TBody>(ref TBody body) where TBody : struct, IForEach<T1, T2, T3, TBody>
    {
        EnsureMatches();
        for (int a = 0; a < _matches.Length; a++)
        {
            ...
            for (int r = 0; r < n; r++)
                TBody.Execute(ref body, archetype.EntityAt(r), ref array1.Get(r), ref array2.Get(r), ref array3.Get(r));
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    public void ForEachParallel<TBody>(JobSystem jobs, ref TBody body, int minChunk) where TBody : struct, IForEach<T1, T2, T3, TBody>
    {
        EnsureMatches();
    ...
    private static class Dispatcher<TBody> where TBody : struct, IForEach<T1, T2, T3, TBody>
    {
        public static readonly QueryParallelDispatch3<T1, T2, T3, TBody> Instance = new();
    }
}
