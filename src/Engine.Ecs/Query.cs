...
public sealed class Query<T1> where T1 : unmanaged
{
    ...
    public void ForEach<TBody>(ref TBody body) where TBody : struct, IForEach<T1, TBody>
    {
        EnsureMatches();
        for (int a = 0; a < _matches.Length; a++)
        {
            ...
            for (int r = 0; r < n; r++) TBody.Execute(ref body, archetype.EntityAt(r), ref array.Get(r));
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    public void ForEachParallel<TBody>(JobSystem jobs, ref TBody body, int minChunk) where TBody : struct, IForEach<T1, TBody>
    {
        EnsureMatches();
    ...
    private static class Dispatcher<TBody> where TBody : struct, IForEach<T1, TBody>
    {
        public static readonly QueryParallelDispatch<T1, TBody> Instance = new();
    }
}
