...
internal sealed class QueryParallelDispatch2<T1, T2, TBody>
    where T1 : unmanaged
    where T2 : unmanaged
    where TBody : struct, IForEach<T1, T2, TBody>
{
    private readonly Action<int, int> _runChunks;
    private Query<T1, T2> _query = null!;
    private TBody _body;
    ...
    public QueryParallelDispatch2() => _runChunks = Run;

    public void Run(JobSystem jobs, ref TBody body, int minChunk, Query<T1, T2> query)
    {
        _query = query;
        _body = body;
    ...
    private void Run(int lo, int hi)
    {
        ...
        for (int c = lo; c < hi; c++)
        {
            ...
            for (int r = start; r < end; r++)
                TBody.Execute(ref _body, archetype.EntityAt(r), ref array1.Get(r), ref array2.Get(r));
        }
    }
}
