...
public static class SpriteExtraction
{
        ...
        float jumpHeight = 0f)
    {
        ...
        for (int i = start; i < written; i++)
        {
            ...
            sprites[i] = sprites[i] with { Position = sprites[i].Position + item.ScreenOffset, Size = sprites[i].Size * item.Scale, Color = new Vector4(color.X, color.Y, color.Z, color.W * item.Opacity), Material = item.Material, Scale = item.Scale, AnimationFrame = item.AnimationFrame, UvScale = item.UvScale, UvOffset = item.UvOffset, Blend = item.Blend };
        }
        return written;
    }
