...
using IsometricSandbox.Game.Gameplay.Progression;
...
namespace IsometricSandbox.Game.App;
...
public sealed class ArcherGameApp : GameHost, IDisposable
{
    ...
    private static readonly WeaponDefinition Bow = new(ItemIds.GoblinSlayerBow, 1f, SampleConfig.ArrowSpeed, SampleConfig.ArrowLifetime);
    ...
    {
        GameApplicationOptions applicationOptions = new(
            SampleConfig.WindowTitle,
            new Vector2(SampleConfig.WindowWidth, SampleConfig.WindowHeight),
            options.FrameCap,
            SampleConfig.ArpgSpriteCapacity,
            options.StartFullscreen,
            options.ShowMetrics);
        IGameApplicationBackend backend = GamePlatform.CreateBackend(applicationOptions);
        return new ArcherGameApp(options, backend);
    }

    private ArcherGameApp(Options options, IGameApplicationBackend backend)
        : base(BuildConfig(options), backend)
    {
        _backend = backend;
        _jobs = Jobs;
        _scheduler = new FrameScheduler(_jobs);
        ...
        _textures = new TextureLibrary(Render);
        _font = new SplashFont(Render, Path.Combine(AppContext.BaseDirectory, "textures", "splash-font.png"));
    ...
    protected override void OnResize()
    {
        ...
        Render.Resize((int)Viewport.X, (int)Viewport.Y);
    }

    protected override void OnPerFrame()
    {
        InputActionMapper.CaptureCurrent(GameInput, ref _inputActions, _inputBindings);
