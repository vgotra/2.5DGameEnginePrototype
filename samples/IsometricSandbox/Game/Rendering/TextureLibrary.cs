...
public sealed class TextureLibrary(RenderContext renderer) : ITileTextureProvider, IDisposable
{
    private readonly RenderContext _renderer = renderer;
    ...
    public void LoadNextStep()
    {
        ...
        TextureHandle texture = _renderer.UploadTexture(image.Value.Data, image.Value.Width, image.Value.Height, TextureFilter.Nearest);
    ...
    public TextureHandle UploadGltfAtlas(in GltfSpriteAtlas atlas)
    {
        ...
        TextureHandle handle = _renderer.UploadTexture(atlas.Rgba, atlas.Width, atlas.Height, TextureFilter.Nearest);
    ...
    private void ApplyFallback(int step)
    {
        switch (step)
        {
            case 0: Player = TrackOwned(ProceduralTextures.UkraineFlag(_renderer)); break;
            case 1: Deer = TrackOwned(ProceduralTextures.Blob(_renderer, new(0.3f, 0.6f, 0.35f, 1f))); break;
            case 2: Rabbit = TrackOwned(ProceduralTextures.Blob(_renderer, new(0.95f, 0.55f, 0.65f, 1f))); break;
        }
    }
    ...
    {
        TextureHandle handle = _renderer.UploadTexture(rgba, width, height, filter);
        _ownedTextures.Add(handle);
        return handle;
    }
    ...
    {
        if (_ownedTextures.Remove(handle)) _renderer.ReleaseTexture(handle);
    }

    public void Dispose()
    {
        foreach (TextureHandle handle in _ownedTextures) _renderer.ReleaseTexture(handle);
