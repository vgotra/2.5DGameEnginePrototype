...
public sealed class GameProgression
{
    ...
    private void UpdateCompanionIntent()
    {
        ...
        if (state.Intent.Kind == AiIntentKind.Follow)
        {
            ...
            CharacterIntent follow = new(CharacterIntentKind.MoveTo, default, playerPosition, default, default, default);
    ...
    private void TravelToForest()
    {
        ...
        _companion = _world.SpawnNpc(new NpcDefinition(3, scene.Map.Resolve("cleric-companion").Position, 1.4f, 0.35f, 20, default, new System.Numerics.Vector2(34, 44), new System.Numerics.Vector4(0.4f, 0.8f, 1f, 1f)));
    ...
    private void TickForestCombat()
    {
        if (_tick < _nextCombatTick) return;
        _nextCombatTick += 30;
        for (int i = 0; i < _enemies.Length; i++)
        {
            Entity enemy = _enemies[i];
            if (!enemy.IsValid || !_world.EcsWorld.IsAlive(enemy)) continue;
            ref Health health = ref _world.EcsWorld.Get<Health>(enemy);
            health.Value = 0;
            EnemyId enemyId = i == 0 ? GameContent.GoblinWarrior : i == 1 ? GameContent.GoblinArcher : GameContent.GoblinShaman;
            QuestRuntime.RecordKill(ref Quest, enemyId, in _quest);
            _world.Commands.Destroy(enemy);
            _enemyCount--;
        }
