...
Windows-first (at current moment) .NET 10 isometric game engine prototype. Vulkan is the renderer. Before starting work, read `docs/README.md` (the docs index — read only this index file with its links; open topic files only when you really need their details for the task) and `.agents/context/*` (CurrentState, KnownIssues) and update `.agents/context/` after each milestone — those files are the resumable-state convention; keep those updates brief (terse bullets only), since they are auto-loaded into every session's instructions. For the codebase structure map, use the `mcp-repo-graph` MCP server (no FilesMap file). The repo root already contains `opencode.json` (opencode config: `instructions` auto-loads `.agents/context/*`; MCP registers the `vulkan` and `mcp-repo-graph` servers — see `docs/AgentTooling.md`).
