...
Windows-first (at current moment) .NET 10 isometric game engine prototype. Vulkan is the renderer. Before starting work, read `docs/README.md` (docs index), `.agents/context/FilesMap.md` (repo map), the relevant topic docs, and `.agents/context/*` (CurrentState, NextSteps, Decisions, KnownIssues) and update `.agents/context/` plus `RELEASE_NOTES.md` after each milestone — those files are the resumable-state convention. The repo root already contains `opencode.json` (opencode config: `instructions` auto-loads `.agents/context/*`; MCP registers the `vulkan` + `renderdoc` servers — see `docs/AgentTooling.md`).
...
- Do NOT negate Y in `shape.vert.glsl`: screen space is y-down and `gl_Position` stores `ndc.y` unchanged (Vulkan NDC). Flipping it silently breaks the NDC Y convention.
...
- `samples/IsometricSandbox/Program.cs` is the executable vertical slice (top-level statements, single Vulkan game loop); it talks only to platform contracts + `Engine.Platform.Desktop`, never to Win32 directly. `tests/Engine.Tests` references the sample project to exercise game code (`MovementSystem`, `RenderExtractionSystem`, camera).
- Vulkan is the only renderer (white/black diamonds in iso, boxes in `--2d`); changing `IRenderer` means updating `VulkanRenderer`.

## Platforms
- Windows-first: Vulkan is the renderer. Linux (X11/Wayland via SDL2) and macOS are planned, later — see `docs/LinuxSupportPlan.md`.
