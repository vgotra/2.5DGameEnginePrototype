...
## 2026-08-02
- Removed the GDI reference/fallback renderer: deleted `Win32TileRenderer.cs`, collapsed `samples/IsometricSandbox/Program.cs` to the single Vulkan game loop, dropped the `--gdi`/`--vulkan` flags (only `--2d`/`--fullscreen` remain), and removed the dirty-gated repaint machinery (`ConsumeRepaint`/`WM_PAINT` flagging) and the `WM_ERASEBKGND` suppression from `IGameWindow`/`Win32Window` (a no-op for the STATIC-class window, which has no background brush). Vulkan is now the only renderer on every platform. Updated AGENTS.md, README.md, all GDI references across `docs/`, and the resumable-state context (`.agents/context/*`, `RELEASE_NOTES.md`).
